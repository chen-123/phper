...
function get_replication_tree($array,$host='---',$port='---',$level=0){
    ...
    foreach ( ( array ) $array as $v ) {
        if(((is_string($host) and $v['master_server']==$host) or (is_array($host) and in_array($v['master_server'],$host))) and $v['master_port']==$port)
        {
			if($v['host'] == $v['alias_host']){
            	$host_v = $v['host'];
			}else{
				$host_v = array($v['host'],$v['alias_host']);
			}
            $port_v=$v['port'];
