...
class Servers extends Front_Controller {
    ...
    public function add(){
		...
		if(isset($_POST['submit']) && $_POST['submit']=='add')
        {
			...
			$this->form_validation->set_rules('alias_host',  'lang:alias_host', 'trim|required|min_length[6]|max_length[30]');
			...
			else
			{
					...
					$data = array(
						...
						'alias_host'=>$this->input->post('alias_host'),
						...
						'idc_type'=>$this->input->post('idc_type'),
						'alarm_time'=>$this->input->post('alarm_time'),
                        'threshold_starttime'=>$this->input->post('threshold_starttime'),
                        'threshold_endtime'=>$this->input->post('threshold_endtime'),
					);
              ...
              if(!empty($host) && !empty($port) && !empty($application)){
                 $data['error_code']=0;
					$data = array(
						'host'=>$this->input->post('host_'.$n),
						'port'=>$this->input->post('port_'.$n),
						'alias_host'=>$this->input->post('alias_host_'.$n),
						'idc_type'=>$this->input->post('idc_type_'.$n),
