...
<form name="form" class="form-inline" method="get" action="<?php site_url('monitor/replication') ?>" >
  ...
  <select name="idc_type" class="input-medium" style="">
  <option value="">选择机房</option>
  <option value="yf" <?php if($setval['idc_type']=='yf') echo "selected"; ?> >西街机房</option>
  <option value="ts" <?php if($setval['idc_type']=='ts') echo "selected"; ?> >新村机房</option>
  </select>  
 ...
 <?php foreach ($datalist  as $item):?>
    <tr style="font-size: 12px;">
        ...
        <?php if($option_kill_process==1){ ?>
            ...
            <div class="<?php echo $item['pid'] ?>"><a href="javascript::(0)" ondblclick="kill_process(<?php echo $item['server_id'] ?>,<?php echo $item['pid'] ?>,'<?php echo $item['idc_type'] ?>')" class="btn-danger btn-mini  btn">结束进程</a></div>
            <?php }else{ ?>
...
<script type="text/javascript">
function kill_process(server_id,pid,idc_type){
    if(server_id=='' || pid==''){
        alert("进程异常，请重试！");
    }
    else{
        $.ajax({   
		  type:"GET",   
			  ...
			  data:{
				  server_id: server_id, pid: pid ,idc_type: idc_type
				  },   
			  beforeSend:function(){
