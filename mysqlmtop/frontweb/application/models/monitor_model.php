...
class Monitor_model extends CI_Model{
    ...
    function get_total_record_status(){
        ...
        !empty($_GET["idc_type"]) && $this->db->where("status.idc_type", $_GET["idc_type"]); 
	...
	!empty($_GET["idc_type"]) && $this->db->where("servers.idc_type", $_GET["idc_type"]); 
    ...
    function get_total_record_replication(){
        ...
        $this->db->select('repl.*,servers.host,servers.alias_host,servers.port,application.display_name application');
        $this->db->from('mysql_replication repl');
        $this->db->join('servers', 'repl.server_id=servers.id', 'left');
        $this->db->join('application', 'servers.application_id=application.id', 'left');
       
	!empty($_GET["idc_type"]) && $this->db->where("servers.idc_type", $_GET["idc_type"]);
        !empty($_GET["application_id"]) && $this->db->where("repl.application_id", intval($_GET["application_id"]));
