...
def check_mysql_widget(host,port,user,passwd,server_id,idc_type='ts'):
	...
	cur.execute("select VARIABLE_VALUE  from GLOBAL_STATUS where VARIABLE_NAME ='Qcache_hits';");
        ...
        result=datalist
        if result:
             sql="insert into mysql_widget_hit_rate(server_id,Query_cache_hits,Key_buffer_read_hits,Key_buffer_write_hits,Thread_cache_hits,Key_blocks_used_rate,Created_tmp_disk_tables_rate,idc_type) values(%s,%s,%s,%s,%s,%s,%s,%s)"
             param=(server_id,result[0],result[1],result[2],result[3],result[4],result[5],idc_type)
